 ...
 * Fields: 'text','password', 'textarea', 'email', 'checkbox', 'radio', 'select',
 *          'countries','kvselect','multiselect','multiple', 'date','date-special',
 *          'image', 'range', 'wp_color','wp_upload','submit'
 * * * * */
class FormBuilder
{
    ...
    public $args_help = array(
        ...
        'NOTE FOR THE ABOVE 3' => 'IF ANY ONE OF THE name/id/label WAS <b>NOT</b> PROVIDED, IT WILL <b>NOT</b> WORK IN THIS VERSION AS THIS VERSION(3) DEALS WITH WORDPRESS<hr/>',
        ...
        'Available types in version 3' => "'text','password', 'textarea', 'email', 'checkbox', 'radio', 'select',
                                            'countries','kvselect','multiselect','multiple', 'date','date-special',
                                            'image', 'range', 'wp_color','wp_upload','submit'",
    );
    ...
    public function field($args)
    {
        ...
        $types = array('text','password', 'textarea', 'email', 'checkbox', 'radio', 'select',
                        'countries','kvselect','multiselect','multiple', 'date','date-special',
                         'image', 'range', 'wp_color','wp_upload','submit');

        if ((!isset($name) || empty($name) || $name =="") ){ echo "<h4 class='text-danger form-group row'>You must specify an name for this field to work!.</h4>"; return;}
        elseif ((!isset($id) || empty($id) || $id =="") ){ echo "<h4 class='text-danger form-group row'>You must specify an id for this field to work!.</h4>"; return;}
        elseif ((!isset($label) || empty($label) || $label =="") ){ echo "<h4 class='text-danger form-group row'>You must specify a label for this field to work!.</h4>"; return;}
        ?>
            <?php
        if ($type === "range") {
            if (($min == "") || ($min == "")) {
        ...
        $the_id = $id; $the_label = $label; $the_name = $name;
        ?>
